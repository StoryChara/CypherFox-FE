...
const CardNav = ({
  ...
  // Crear timeline inicial y limpiar estado al desmontar
  useLayoutEffect(() => {
    ...
    return () => {
      tl?.kill();
      tlRef.current = null;
      // seguridad: si el componente se desmonta, deja el estado cerrado
  ...
  // Manejo de resize: reconstruye el timeline desde estado cerrado
  useLayoutEffect(() => {
    const handleResize = () => {
      const tl = tlRef.current;
      if (!tl) return;

      // Forzar visualmente el nav a cerrado
  ...
  const hardCloseMenu = () => {
    // Cierra el menú sin depender de la animación
    setIsHamburgerOpen(false);
    setIsExpanded(false);

    const tl = tlRef.current;
    if (tl) {
      tl.pause(0); // vuelve al inicio del timeline
    }
    if (navRef.current) {
      gsap.set(navRef.current, { height: 60 });
    }
  ...
  const toggleMenu = () => {
    ...
    if (!isExpanded) {
      ...
      setIsExpanded(true);
      tl.eventCallback('onReverseComplete', null); // limpiar callback previo por seguridad
      tl.progress(0).play();
    } else {
      ...
      >
        <div className="card-nav-top">
          ...
          <div className="logo-container">
            <Link to="/" className="logo-link" onClick={hardCloseMenu}>
              <img src={logoProp} alt={logoAlt} className="logo-img" />
